...
def compute_dual_sensitivity(forget_sensitivity, retain_sensitivity, epsilon=1e-6):
    """
    Calculate unlearning importance score.
    
    Args:
        forget_sensitivity: Sensitivity scores from forget set
        retain_sensitivity: Sensitivity scores from retain set
        epsilon: Small constant to avoid division by zero
        
    Returns:
        Unlearning importance score: s_forget/(s_retain + epsilon)
    """
    return forget_sensitivity / (retain_sensitivity + epsilon)
...
def unlearning_prune(model, dual_sensitivity_dict, ratio, unlearning_threshold):
    """
    Prune weights based on unlearning scores while maintaining minimum performance on retain set.
    
    Args:
        model: The model to prune
        dual_sensitivity_dict: Dictionary of dual sensitivity scores
        ratio: Target pruning ratio
        unlearning_threshold: Threshold for unlearning score to consider pruning
    """
    original_param_num = 0
    pruned_param_num = 0
    for name, module in model.named_modules():
        if _is_target_larer(module):
            ...
            # Only prune if unlearning score is above threshold
            importance = dual_sensitivity_dict[name] * c_mask
            prune_candidates = torch.where(importance > unlearning_threshold)[0]
            
            if len(prune_candidates) > 0:
                # Sort by importance among candidates that exceed threshold
                can_prune = prune_candidates[torch.argsort(importance[prune_candidates])][:need_prune_num]
                mask[can_prune] = 0
